...
momo 每次请求该地址时会自动同步上游来源（存在上游来源时），同一来源 60 秒最小间隔去重；可加 `?refresh=0` 跳过。同步失败的来源降级返回缓存节点，并带 `X-Upstream-Warning` 响应头。
...
- Momo / Kernel 导出**默认自动同步**上游（每来源 60 秒最小间隔；加 `?refresh=0` 跳过）
