...
# run Docker container (default ADMIN_PASSWORD=admin)
docker-run:
    docker run -d --name subhatch -p 3000:3000 -v subhatch-data:/data -e ADMIN_PASSWORD=${ADMIN_PASSWORD:-admin} -e SUB_TOKEN=${SUB_TOKEN:-} --restart unless-stopped subhatch
